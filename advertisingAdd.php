...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "insert_advertising")) {
	
	move_uploaded_file($_FILES["pic"]["tmp_name"],"pictures\advertising\\" .$_FILES["pic"]["name"]);  $insertSQL = sprintf("INSERT INTO advertising (title, website, content, pic) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['title'], "text"),
                       GetSQLValueString($_POST['website'], "text"),
                       GetSQLValueString($_POST['content'], "text"),
                       GetSQLValueString("administrator/pictures/advertising/" . $_FILES["pic"]["name"], "text"));

  mysql_select_db($database_dbconnect, $dbconnect);
  $Result1 = mysql_query($insertSQL, $dbconnect) or die(mysql_error());

  $insertGoTo = "advertising.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    ...
    <div id="tabs">
         <div class="container">
            <ul>
                      ...
                      <li><a href="advertisingAdd.php" class="current"><span>新增友站連結資料</span></a></li>
...
<form id="insert_advertising" name="insert_advertising" action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data">
            	<table width="100%">
                  <tr >
                    <td width="15%">廣告標題</td>
                    <td><input name="title" type="text" size="50"  weight="300"/></td>
                  </tr>
				  <tr><td>&nbsp;</td></tr>
				  <tr>
				    <td>圖片</td>
					<td><input type="file"  id="pic" name="pic" weight="20%" /></td>
				  </tr>
				  <tr><td>&nbsp;</td></tr>
				  <tr>
				    <td>連結</td>
					<td><input type="text" name="website" weight="20%" /></td>
				  </tr>
                </table>
